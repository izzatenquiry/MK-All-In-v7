...
echo.
echo [INFO] Verifying .env.monoklix file...
if exist "%~dp0.env.monoklix" (
    echo [SUCCESS] .env.monoklix found
) else (
    echo [WARNING] .env.monoklix not found! Creating it...
    echo VITE_BRAND=monoklix > "%~dp0.env.monoklix"
    echo [SUCCESS] .env.monoklix created
)
echo.
...
echo [INFO] Starting React App (MONOKLIX)...
start "React 8080" /d "%~dp0" cmd /k "npm run dev:monoklix & pause"
