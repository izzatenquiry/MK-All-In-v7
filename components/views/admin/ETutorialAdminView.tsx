...
import { ImageIcon, TrashIcon, ChevronDownIcon, PencilIcon } from '../../Icons';
...
const PlatformUpdatesPanel: React.FC = () => {
    ...
    const handleAddAnnouncement = async () => {
        ...
        // Dispatch event to refresh announcements in App.tsx
        eventBus.dispatch('announcementsUpdated');
    };
    
    const handleEditAnnouncement = (ann: Announcement) => {
        setNewAnnouncement({ title: ann.title, content: ann.content, category: ann.category });
        setEditingId(ann.id);
        // Scroll to form
        document.querySelector('[data-announcement-form]')?.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
    };
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="p-4 border border-neutral-200 dark:border-neutral-700 rounded-md space-y-4">
                ...
                <div className="space-y-3 p-3 bg-neutral-50 dark:bg-neutral-800/50 rounded-md" data-announcement-form>
                    <h4 className="font-medium">{editingId ? 'Edit Announcement' : 'Post New Announcement'}</h4>
                    ...
                    {announcements.map(ann => (
                        <div key={ann.id} className="flex justify-between items-start p-3 bg-neutral-50 dark:bg-neutral-800/50 rounded-md">
                            ...
                            <div className="flex gap-2">
                                <button onClick={() => handleEditAnnouncement(ann)} className="p-2 text-blue-500 hover:text-blue-700" title="Edit">
                                    <PencilIcon className="w-4 h-4" />
                                </button>
                                <button onClick={() => handleDeleteAnnouncement(ann.id)} className="p-2 text-red-500 hover:text-red-700" title="Delete">
