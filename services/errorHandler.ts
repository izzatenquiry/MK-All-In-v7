...
export const handleApiError = (error: unknown): string => {
    ...
    const isDefinitiveAuthFailure = 
        ...
        lowerCaseMessage.includes('unauthenticated') || // Add UNAUTHENTICATED status
        lowerCaseMessage.includes('unauthorized') || // Add unauthorized
        message.includes('[401]') || // Add status code check
        message.includes('ERROR 401') || // Check for our custom 401 error message
    ...
    if (isDefinitiveAuthFailure) {
        // Log helpful message to console for user visibility
        console.error('🔑 ERROR 401 - Your token is invalid or has expired.');
        console.error('💡 Please go to Settings > Token Setting to generate a new token.');
        eventBus.dispatch('personalTokenFailed');
        // Return the full error message if it contains our custom message, otherwise return tokenInvalid
        if (message.includes('ERROR 401')) {
            return message; // Return the full custom message
        }
        return 'tokenInvalid';
    }
